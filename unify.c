...
bool unify(struct type *ix, struct type *iy) {
    ...
    while (Sx.top && Sy.top /*not_empty(Sx) and not_empty(Sy)*/) {
        ...
        if (!isvar(i) && !isvar(j)) { // case 1: i is bound to a term and j is bound to a term}
            ...
            struct type *b=j->args[0];//find(mgu_begin,j->name); //dereference j
            if (!b/*j is a free variable*/) j->args[0]=i;//mgu_begin=append(mgu_begin,j->name,i); //bind j to i and set mgu[j] = 1
            else { // j is bound
                if (!isvar(b)) {
                    typestack_push(&Sx,i); //push i on Sx
                    typestack_push(&Sy,b); //push j on Sy
                } else { // j is bound to variable
                    struct type *bb=b->args[0];//find(mgu_begin,b->term->name); //deref(deref(j))
                    if (bb) {
                        typestack_push(&Sx,i); //push i on Sx
                        typestack_push(&Sy,bb); //push j on Sy
                    } else j->args[0]=i;//mgu_begin=append(mgu_begin,j->name,i); // bind j to i
                }
            }
        } else if (isvar(i) && !isvar(j)) { // case 3: i is bound to a variable and j is bound to a term
            // perfectly symmetric to case 2
            struct type *b=i->args[0];//find(mgu_begin,i->name); //dereference i
            if (!b/*i is a free variable*/) i->args[0]=j;//mgu_begin=append(mgu_begin,i->name,j); //bind i to j and set mgu[i] = 1
            else { // i is bound
                if (!isvar(b)) {
                    ...
                    } else i->args[0]=j;//mgu_begin=append(mgu_begin,i->name,j); // bind j to i
                }
            }
        } else if (isvar(i) && isvar(j)) { // case 4: i is bound to a variable and j is bound to a variable
            struct type *fi=i->args[0];//find(mgu_begin,i->name);
            struct type *fj=j->args[0];//find(mgu_begin,j->name);
            if (!fi && !fj/*i is free and j is free*/) {
                i->args[0]=j;//mgu_begin=append(mgu_begin,i->name,j);
                ...
                if (!isvar(fj) || strcmp(fj->name,i->name)) //if we do not bind X to deref(Y) if deref(Y)=X
                    i->args[0]=fj;//mgu_begin=append(mgu_begin,i->name,fj->term);
            } else if (fi && !fj/*i is bound and j is free*/) {
                if (!isvar(fi) || strcmp(fi->name,j->name))
                    j->args[0]=fi;//mgu_begin=append(mgu_begin,j->name,fi->term);
